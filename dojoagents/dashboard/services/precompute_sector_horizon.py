    ...
    *,
    ...
    if fundamentals_period is not None and not fundamentals_period.empty:
        ...
        for key, group in fin.groupby(["market", "level1_id", "level2_id", "level3_id"], sort=False):
            fin_by_theme[(str(key[0]), str(key[1]), str(key[2]), str(key[3]))] = (
                summarize_multi_quarter_fundamentals(group)
            )
    ...
    for key, group in frame.groupby(group_cols, sort=True):
        ...
        levels = [
            _finite(value) if _finite(value) is not None else float("nan")
            for value in series["index_level"].tolist()
        ]
        daily_rets = [
            _finite(value) if _finite(value) is not None else float("nan")
            for value in series["daily_return_pct"].tolist()
        ]
        ...
        if bench is not None and not bench.empty:
            bench_map = {
                str(row.trade_date): _finite(row.daily_return_pct)
                for row in bench.itertuples(index=False)
            }
            bench_rets = [bench_map.get(day) for day in dates]

        fin_payload = fin_by_theme.get((market, l1, l2, l3)) or summarize_multi_quarter_fundamentals(
            pd.DataFrame()
        )
        ...
        for idx in range(len(series)):
            ...
            for days in HORIZON_WINDOWS:
                ...
                market_ret = (
                    _compound_return_pct(bench_window, days) if bench_window else None
                )
                rs = (
                    sector_ret - market_ret
                    if sector_ret is not None and market_ret is not None
                    else None
                )
    ...
    filled_parts = [
        _fill_cross(group)
        for _, group in out.groupby(["market", "trade_date"], sort=False)
    ]
    out = pd.concat(filled_parts, ignore_index=True) if filled_parts else out
    out = out[SECTOR_HORIZON_METRICS_COLUMNS].sort_values(
        ["trade_date", "market", "level1_id", "level2_id", "level3_id"]

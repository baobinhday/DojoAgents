...
def _primary_constituent_map(constituents: pd.DataFrame) -> pd.DataFrame:
    ...
    if constituents is None or constituents.empty:
        return pd.DataFrame(
            columns=["market", "ticker", "level1_id", "level2_id", "level3_id", "role", "market_cap", "pe"]
        )
    ...
    return (
        frame.drop_duplicates(subset=["market", "ticker"], keep="first")[
            ["market", "ticker", "level1_id", "level2_id", "level3_id", "role", "market_cap", "pe"]
        ]
...
def _benchmark_window_returns(benchmark_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in frame.groupby("market", sort=False):
        ...
        out_parts.append(
            g[["market", "trade_date", *[f"mkt_ret_{w}d" for w in (*SHORT_WINDOWS, *MID_WINDOWS)]]]
        )
    return (
        pd.concat(out_parts, ignore_index=True)
        if out_parts
        else pd.DataFrame(columns=["market", "trade_date"])
    )
...
def _sector_window_returns(sector_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        if parts
        else pd.DataFrame(columns=["market", "level1_id", "level2_id", "level3_id", "trade_date"])
    )
...
def compute_ticker_alpha_factors_frame(
    *,
    ...
    td["s_val_pe"] = pe
    td["m_val_pe"] = pe
    td["s_val_pe_cheap_cs"] = td.groupby(["trade_date", "market"], sort=False)["pe"].transform(
        _cross_section_pe_cheap
    )

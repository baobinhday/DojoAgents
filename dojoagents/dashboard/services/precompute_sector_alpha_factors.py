...
def compute_structure_daily_factors(
    *,
    ...
    if (
        ...
        or constituents.empty
        or ticker_daily is None
        or ticker_daily.empty
        or sector_daily is None
        or sector_daily.empty
    ):
    ...
    disp = (
        members.groupby(["trade_date", "market", "level1_id", "level2_id", "level3_id"], sort=False)[
            "daily_return_pct"
        ]
    ...
    for key_vals, group in merged.groupby(group_cols, sort=False):
        ...
        member_tuples = [
            (str(r.ticker), float(r.market_cap), float(r.daily_return_pct) if pd.notna(r.daily_return_pct) else None)
        ...
        conc_rows.append(
            {
                ...
                "level3_id": l3,
                "s_struct_leader_conc": (
                    float(payload.leader_concentration_pct) if payload is not None else float("nan")
                ),
            }
        )
...
def _map_theme_horizon_factors(
    ...
    if horizon_df is not None and not horizon_df.empty:
        ...
        if "link_key_h" in base.columns:
            base["link_key"] = base["link_key"].where(
                base["link_key"].astype(str).str.len() > 0, base["link_key_h"]
            )
...
def compute_sector_alpha_factors_frame(
    *,
    ...
    if (
        ...
        and ticker_daily is not None
        and sector_daily is not None
        and not constituents.empty
    ...
    if "factor_universe_size" in factors.columns:
        factors["factor_universe_size"] = factors.groupby(["trade_date", "market"], sort=False)[
            "level3_id"
        ].transform("size")

...
@router.get(
    "/return-curve",
    response_model=SectorReturnCurveResponse,
    operation_id="get_sector_return_curve",
    summary="Sector rebased NAV curve and companion daily series for one market window",
)
async def sector_return_curve(
    level1_id: str = Query(..., min_length=1),
    level2_id: str = Query(..., min_length=1),
    level3_id: str = Query(..., min_length=1),
    market: str = Query(..., pattern="^(cn|sh|hk|us)$"),
    start_date: Optional[str] = Query(None, description="Inclusive window start YYYY-MM-DD; requires end_date"),
    end_date: Optional[str] = Query(None, description="Inclusive window end YYYY-MM-DD; requires start_date"),
    ...
    days: Optional[int] = Query(
        ...
        ge=1,
        le=90,
        description="Trading-session count ending at as_of (default 1). Ignored for start_date+end_date.",
    ),
    scope: Literal["L1", "L2", "L3"] = Query("L3"),
    registry=Depends(get_financial_registry),
) -> SectorReturnCurveResponse:
    ...
    try:
        return await build_sector_return_curve_v1(
            registry,
...
async def sector_constituents(
    ...
    scope: Literal["L1", "L2", "L3"] = Query("L3"),
    days: int = Query(1, ge=1, le=90, description="Trading-session count (latest N, or last N ≤ as_of)"),
